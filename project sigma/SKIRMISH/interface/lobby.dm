		...
		update_grid()
			for(var/mob/player/p in players)
				...
				for(var/mob/player/m in participants)							//	Searches through the contents list
					p << output("<center>Lv.[m.level] <font color = [m.namecolor]>[m] -- participating","lobbygrid:1,[++i]")
				for(var/mob/player/m in spectators)							//	Searches through the contents list
					p << output("<center>Lv.[m.level] <font color = [m.namecolor]>[m] -- spectating","lobbygrid:1,[++i]")