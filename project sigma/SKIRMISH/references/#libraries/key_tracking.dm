...
mob/player
	verb
		key_down( key as text )
			set hidden = 1
			if(key == "SHIFT" && active_game.started == 2 && !move_disabled)
				sprint()
				return
			if(active_input)
				if(key == "SHIFT")
					...
					if(!backspace_timeout && active_input.text2disp)
						...
						if(active_input.is_password)
							...
							for(var/i=1, i <= length(active_input.text2disp), i++)
								...
								pass_disp = "[tmpdisp]*"
							active_input.maptext	= "<p class='vendor'>[pass_disp] <"
						else
							active_input.maptext	= "<p class='vendor'>[active_input.text2disp] <"
						sleep;backspace_timeout = 0
					//if(active_input == get_ibox("Username"))
					return
				...
				if(length(active_input.text2disp) < active_input.max_length)
					...
					else active_input.text2disp	= "[active_input.text2disp][lowertext(key)]"
					...
					if(active_input.is_password)
						...
						for(var/i=1, i <= length(active_input.text2disp), i++)
							var/tmpdisp = "[copytext(pass_disp,1)]"
							pass_disp = "[tmpdisp]*"
						active_input.maptext = "<p class='vendor'>[pass_disp]</font> <"
					else
						active_input.maptext = "<p class='vendor'>[active_input.text2disp] <"
				//if(active_input == get_ibox("Username")) Username = active_input.text2disp
